(() => {
  ...
  function ventDialog({ title = 'Vent', message = '', label = 'Message', value = null, confirmText = 'OK', cancelText = 'Cancel', danger = false } = {}) {
    ...
    const cancel = document.getElementById('ventSiteDialogCancel');
    const confirmButton = document.getElementById('ventSiteDialogConfirm');
    document.getElementById('ventSiteDialogTitle').textContent = title;
    document.getElementById('ventSiteDialogMessage').textContent = message;
    document.getElementById('ventSiteDialogLabel').textContent = label;
    ...
    return new Promise(resolve => {
      dialog.addEventListener('close', () => resolve(dialog.returnValue === 'confirm' ? (value === null ? true : input.value) : null), { once: true });
      dialog.showModal();
      setTimeout(() => (value === null ? confirmButton : input).focus(), 20);
    });
  }
  const ventMessage = (message, title = 'Vent') => ventDialog({ title, message, cancelText: null });
  ...
  async function notificationUsername(userId) {
    if (!userId) return 'Someone';
    const result = await db.from('profiles').select('username').eq('id', userId).maybeSingle();
  ...
  function bindThreadActions(post) {
    content().querySelector('[data-like]')?.addEventListener('click', async () => {
      ...
      if (result.error) return ventMessage(result.error.message, 'Could not update like'); renderThread(post.id);
    });
    content().querySelectorAll('[data-report-post],[data-report-reply]').forEach(button => button.addEventListener('click', async () => { if (!session) return focusLogin(); const reason = await ventPrompt('Why are you reporting this?', '', 'Report message'); if (!reason?.trim()) return; const payload={reporter_id:session.user.id,reason:reason.trim()}; if (button.hasAttribute('data-report-post')) payload.post_id=Number(button.dataset.reportPost); else payload.reply_id=Number(button.dataset.reportReply); const result=await db.from('reports').insert(payload); await ventMessage(result.error ? result.error.message : 'Report sent privately to moderators.', result.error ? 'Could not send report' : 'Report sent'); }));
    content().querySelectorAll('[data-edit-post],[data-edit-reply]').forEach(button => button.addEventListener('click', async () => { const isPost=button.hasAttribute('data-edit-post'); const id=Number(isPost?button.dataset.editPost:button.dataset.editReply); const current=isPost?post.body:button.closest('.vent-message').querySelector('.vent-message-body').childNodes[0].textContent; const body=await ventPrompt('Edit message',current,'Edit message'); if(!body?.trim())return; const result=await db.from(isPost?'posts':'replies').update({body:body.trim(),is_edited:true,updated_at:new Date().toISOString()}).eq('id',id); if(result.error)await ventMessage(result.error.message,'Could not edit message'); else renderThread(post.id); }));
    content().querySelectorAll('[data-delete-post],[data-delete-reply]').forEach(button => button.addEventListener('click', async () => { if(!await ventConfirm('Delete this permanently?'))return; const isPost=button.hasAttribute('data-delete-post'); const id=Number(isPost?button.dataset.deletePost:button.dataset.deleteReply); const result=await db.from(isPost?'posts':'replies').delete().eq('id',id); if(result.error)await ventMessage(result.error.message,'Could not delete message'); else isPost?renderHome():renderThread(post.id); }));
    content().querySelector('[data-lock]')?.addEventListener('click',async()=>{const result=await db.from('posts').update({is_locked:!post.is_locked}).eq('id',post.id);if(result.error)await ventMessage(result.error.message,'Could not update thread');else renderThread(post.id);});
  }
